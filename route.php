...
if (isset($_POST["connect"])){
  if (($dataAccount[0]['username']==$usernameLogin) && ($dataAccount[0]['password']==$passwordLogin)
    ...
    $loginFile = file_get_contents('./loged.twig', true); // HTML quand connecté
    $isConnected=true;
    ...
    $loginFile = file_get_contents('./login.twig', true); // HTML quand deconnecte
  }
}
...
if ($_POST['disconnect']){
  ...
  $loginFile = file_get_contents('./login.twig', true);
  $url = $actual_link = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
  header("Location: ".$url);
}
...
try{
  $page = $_GET['page'];
  if (is_null($page)) {
  echo $twig->render('header.twig', ['Titre' => 'Acupuncture', 'loginFile' => $loginFile ,'isAdmin' => $isAdmin]);
  ...
  elseif ("_$page"=='_pathologies'){
    echo $twig->render('header.twig', ['Titre' => 'Pathologies', 'loginFile' => $loginFile ,'isAdmin' => $isAdmin]);
    echo $twig->render('pathologies.twig', ['data' => $data,'typeSet'=>$typeSet,
      'pathologieSet'=>$pathologieSet,'meridienSet'=>$meridienSet, "isConnected" => $isConnected]);
    echo $twig->render('footer.twig', []);
  }
  elseif ("_$page"=='_inscription'){
    echo $twig->render('header.twig', ['Titre' => 'Inscription', 'loginFile' => $loginFile ,'isAdmin' => $isAdmin]);
    if($succed==1){echo $twig->render('succed.twig',['message' => $message]);} 
    elseif($succed==0){echo $twig->render('erreur.twig',['message' => $message]);}
    echo $twig->render('inscription.twig',['message' => $message]);
    echo $twig->render('footer.twig', []);
  }

  elseif ("_$page"=='_paneladmin' && $isAdmin==True){
    echo $twig->render('header.twig', ['Titre' => 'Panel admin', 'loginFile' => $loginFile ,'isAdmin' => $isAdmin]);
    echo $twig->render('admin.twig', ['userAccount' => $userAccount]);
    echo $twig->render('footer.twig', []);
  }
  else{
    echo $twig->render('header.twig', ['Titre' => 'Acupuncture', 'loginFile' => $loginFile ,'isAdmin' => $isAdmin]);
